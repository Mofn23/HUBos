...
async function getPreferences() {
  if (!preferencesModule && typeof window !== 'undefined') {
    try {
      const pkgName = '@capacitor/preferences';
      preferencesModule = await import(/* webpackIgnore: true */ pkgName);
    } catch {
      // Plugin not available in environment or web fallback
    }
  }
  return preferencesModule?.Preferences || null;
}
...
export const nativeStorage: StateStorage = {
    ...
    try {
      ...
      try {
        ...
        if (localValue) {
          ...
          // Migrate to native Preferences in background if available
          if (Preferences) {
            Preferences.set({ key: name, value: localValue }).catch((migErr: any) => {
  ...
  setItem: async (name: string, value: string): Promise<void> => {
    ...
    try {
      ...
      // Ignore localStorage QuotaExceededError
    }
...
export async function exportAllHubosData(): Promise<Record<string, any>> {
  const keys = ['hubos_main_v1', 'hubos_recomp_v1', 'hubos_subs_v1', 'hubos_schedule_store_v1', 'hubos_aesthetix_v1'];
